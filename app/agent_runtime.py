...
def init_db():
    with db() as c:
        c.executescript('''CREATE TABLE IF NOT EXISTS ai_agents(id TEXT PRIMARY KEY,name TEXT UNIQUE NOT NULL,model TEXT NOT NULL,backend TEXT NOT NULL DEFAULT 'ollama',system_prompt TEXT NOT NULL DEFAULT '',permissions TEXT NOT NULL DEFAULT '[]',workspace TEXT NOT NULL DEFAULT '',active INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL,updated_at TEXT NOT NULL); CREATE TABLE IF NOT EXISTS ai_agent_jobs(id TEXT PRIMARY KEY,agent_id TEXT NOT NULL,task TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'queued',result TEXT NOT NULL DEFAULT '',created_at TEXT NOT NULL,updated_at TEXT NOT NULL,FOREIGN KEY(agent_id) REFERENCES ai_agents(id));''')
