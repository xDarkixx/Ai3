...
def data_key():
    if ENC_KEY_FILE and os.path.exists(ENC_KEY_FILE):
        raw=Path(ENC_KEY_FILE).read_bytes().strip()
    else: raw=os.getenv("AI3_DATA_ENCRYPTION_KEY","").encode()
    if len(raw)!=32: raise RuntimeError("AI3_DATA_ENCRYPTION_KEY must be exactly 32 bytes or supplied by secret file")
    return raw
...
def ensure_ca():
    ...
    cert=x509.CertificateBuilder().subject_name(subject).issuer_name(subject).public_key(key.public_key()).serial_number(x509.random_serial_number()).not_valid_before(now()-timedelta(minutes=1)).not_valid_after(now()+timedelta(days=3650)).add_extension(x509.BasicConstraints(ca=True,path_length=1),critical=True).add_extension(x509.KeyUsage(digital_signature=True,key_encipherment=False,key_cert_sign=True,crl_sign=True,key_agreement=False,data_encipherment=False,key_agreement=False,content_commitment=False,key_encipherment=False),critical=True).sign(key,hashes.SHA256())
...
def install(app:FastAPI):
    ...
    def ca_info():
        ...
        except Exception as e: raise HTTPException(503,"AI3 PKI is not initialized")
    ...
    def issue(body:CertRequest,x_ai3_admin_key:str|None=Header(default=None),x_ai3_admin_session:str|None=Header(default=None)):
        ...
        builder=x509.CertificateBuilder().subject_name(subject).issuer_name(ca_cert.subject).public_key(key.public_key()).serial_number(x509.random_serial_number()).not_valid_before(now()-timedelta(minutes=1)).not_valid_after(now()+timedelta(days=body.days))
        builder=builder.add_extension(x509.BasicConstraints(ca=False,path_length=None),critical=True)
        if body.kind=="server": builder=builder.add_extension(x509.ExtendedKeyUsage([ExtendedKeyUsageOID.SERVER_AUTH]),critical=False)
        else: builder=builder.add_extension(x509.ExtendedKeyUsage([ExtendedKeyUsageOID.CLIENT_AUTH]),critical=False)
        cert=builder.sign(ca_key,hashes.SHA256()); pem=cert.public_bytes(serialization.Encoding.PEM).decode(); private=key.private_bytes(serialization.Encoding.PEM,serialization.PrivateFormat.PKCS8,serialization.NoEncryption()).decode()
