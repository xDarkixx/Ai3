...
DB_PATH=os.getenv("AI3_DB","/data/ai3.db"); PKI_DIR=Path(os.getenv("AI3_PKI_DIR","/data/pki")); KEY_FILE=PKI_DIR/"ca.key.enc"; CERT_FILE=PKI_DIR/"ca.crt"; ENC_KEY_FILE=os.getenv("AI3_DATA_ENCRYPTION_KEY_FILE","/run/secrets/ai3_data_key")
def db():
...
def data_key():
 ...
 if len(raw)==64:
  try: raw=bytes.fromhex(raw.decode())
  except ValueError: pass
 if len(raw)!=32: raise RuntimeError("AI3 data encryption key must be 32 bytes or 64 hex characters")
 return raw
def protect(raw):
 nonce=os.urandom(12); return nonce+AESGCM(data_key()).encrypt(nonce,raw,b"AI3-CA")
def unprotect(blob): return AESGCM(data_key()).decrypt(blob[:12],blob[12:],b"AI3-CA")
def ensure_ca():
 ...
 KEY_FILE.write_bytes(protect(key.private_bytes(serialization.Encoding.DER,serialization.PrivateFormat.PKCS8,serialization.NoEncryption()))); CERT_FILE.write_bytes(cert.public_bytes(serialization.Encoding.PEM)); os.chmod(KEY_FILE,0o600); os.chmod(CERT_FILE,0o644)
def ca_objects():
 ensure_ca(); return serialization.load_der_private_key(unprotect(KEY_FILE.read_bytes()),password=None),x509.load_pem_x509_certificate(CERT_FILE.read_bytes())
def install(app:FastAPI):
 ...
 class CertRequest(BaseModel):
  common_name:str=Field(min_length=1,max_length=128,pattern=r"^[A-Za-z0-9._:@/-]+$"); kind:str=Field(default="client",pattern=r"^(server|client|agent)$"); days:int=Field(default=90,ge=1,le=825)
 def admin(k,s):
  from app.main import require_admin; require_admin(k,s)
 @app.get("/v1/pki/ca")
 def ca_info():
  try: _,cert=ca_objects()
  except Exception as exc: raise HTTPException(503,"AI3 PKI is not initialized") from exc
 ...
 def issue(body:CertRequest,x_ai3_admin_key:str|None=Header(default=None),x_ai3_admin_session:str|None=Header(default=None)):
  admin(x_ai3_admin_key,x_ai3_admin_session)
  ...
  eku=ExtendedKeyUsageOID.SERVER_AUTH if body.kind=="server" else ExtendedKeyUsageOID.CLIENT_AUTH
  cert=x509.CertificateBuilder().subject_name(subject).issuer_name(ca_cert.subject).public_key(key.public_key()).serial_number(x509.random_serial_number()).not_valid_before(now()-timedelta(minutes=1)).not_valid_after(now()+timedelta(days=body.days)).add_extension(x509.BasicConstraints(ca=False,path_length=None),critical=True).add_extension(x509.ExtendedKeyUsage([eku]),critical=False).sign(ca_key,hashes.SHA256())
  ...
  return {"certificate":pem,"private_key":private,"ca_certificate":ca_cert.public_bytes(serialization.Encoding.PEM).decode(),"serial":str(cert.serial_number),"expires_at":cert.not_valid_after_utc.isoformat(),"warning":"Private key is returned once; store it securely."}
