...
if [ ! -r /etc/os-release ]; then echo "Kann das Betriebssystem nicht erkennen."; exit 1; fi
. /etc/os-release
if [ "${ID:-}" != "ubuntu" ] || [ "${VERSION_ID:-}" != "24.04" ]; then
  echo "AI3 One-Click benötigt Ubuntu 24.04 LTS. Erkannt: ${PRETTY_NAME:-unbekannt}"; exit 1
fi
command -v apt-get >/dev/null 2>&1 || { echo "apt-get fehlt."; exit 1; }

echo "[1/8] Installiere alle Basis-Komponenten ..."
apt-get update
...
if command -v nvidia-smi >/dev/null 2>&1; then
  ...
  if [ "$GPU_SETUP_OK" -eq 1 ] && docker run --rm --gpus all nvidia/cuda:12.6.2-base-ubuntu24.04 nvidia-smi >/dev/null 2>&1; then
    GPU_COMPOSE_READY=1; echo "NVIDIA-Docker-GPU-Test erfolgreich."
  else echo "GPU vorhanden, GPU-Container-Test fehlgeschlagen – CPU-Modus."; fi
else
...
import sys
source,target,root=map(Path,sys.argv[1:]); Path(target).write_text(source.read_text(encoding="utf-8").replace("/opt/ai3",str(root)),encoding="utf-8")
PY
...
PY
  ...
  [ -f runtime/update-config.json ] || printf '{"auto_update":true}\n' > runtime/update-config.json
...
echo "[6/8] Führe Abschlussdiagnose aus ..."
./scripts/setup-mail.sh
./scripts/doctor.sh

echo "[7/8] Prüfe Dienste und automatischen Web-Start ..."
docker compose ps
systemctl is-enabled ai3-network-refresh.timer >/dev/null 2>&1 && echo "LAN-Watcher: aktiv" || true
systemctl is-enabled ai3-auto-update.timer >/dev/null 2>&1 && echo "Auto-Updater: aktiv" || true
...
echo "[8/8] Installation abgeschlossen – Control Center wird geöffnet."
...
echo "Web UI:     automatisch geöffnet (wenn grafische Sitzung vorhanden)"
echo "Auto-Update: GitHub-Check alle 15 Minuten"
echo "========================================"
