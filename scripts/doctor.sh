...
check "AI3 HTTPS Health" curl -kfsS https://localhost/health
...
for p in 80 443; do if command -v ss >/dev/null 2>&1 && ss -lnt | grep -q ":$p "; then echo "[OK]   Webport $p lauscht"; else echo "[WARN] Webport $p nicht lokal sichtbar"; fail=1; fi; done
