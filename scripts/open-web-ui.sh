...
# Open the AI3 control center after installation. On headless servers the URL is printed.
...
if command -v xdg-open >/dev/null 2>&1 && [ -n "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ]; then
  if [ -n "${SUDO_USER:-}" ] && id "$SUDO_USER" >/dev/null 2>&1; then
    runuser -u "$SUDO_USER" -- env DISPLAY="${DISPLAY:-}" WAYLAND_DISPLAY="${WAYLAND_DISPLAY:-}" XDG_RUNTIME_DIR="${XDG_RUNTIME_DIR:-/run/user/$(id -u "$SUDO_USER")}" xdg-open "$URL" >/dev/null 2>&1 || true
  else
  ...
  echo "Kein grafischer Browser verfügbar. Öffne die URL auf einem PC im Netzwerk."
fi
